...
| Skill 内部链接 | 10 / 10 有效 |
| 发布版本源 | `podcast-reader/VERSION` = `2.0.1` |
...
- **Bilibili 完整新链路**：用户提供的 `BV1MAut67Epv` 在无 Cookie、无云 API Key 条件下完成 5,899.520 秒音频获取、4 块本地转写、202 个时间戳片段、严格证据分析、阅读器与安全分享包；实测发现并修复了静默截断和中文段内重复幻觉漏检。
