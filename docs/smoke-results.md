...
## FT-v2.1-01：Bilibili 98 分钟完整分析闭环

- URL：<https://www.bilibili.com/video/BV1MAut67Epv/>
- 来源：《李翔对谈单伟建：经历过周期会让人谦逊【详谈】》，页面标称 5,900 秒，无公开字幕。
- 获取：yt-dlp 在当前网络返回 HTTP 412 后，无 Cookie 使用公开 API；首次真实运行发现 CDN 在 1,069.952 秒提前结束却返回正常退出，旧实现会把非空文件误判为完成。
- 修复：改用 8 MiB Range 可恢复下载，核对远端 42,546,145 字节，并在缓存、源文件及转码后三层验证媒体时长；完整音频为 5,899.520 秒。
- 转写：无云 API Key，CPU 本地 tiny 模型处理 4 个可恢复块，生成 202 个连续时间戳片段。
- 质量：新增无空格语言的段内重复检测，准确标出第 44、64 段 ASR 幻觉，质量由错误的 `pass/100` 修正为 `warn/90`。
- 分析：生成 9 章中文深度拆解、11 条结构化主张、3 条逐字短引及 92/202 segment 引用覆盖；Markdown、证据、bundle 和阅读器严格校验全部通过。
- 分享：隐私安全 ZIP 默认不含完整逐字稿；刷新质量报告后的 SHA-256 为 `c5c8af8369df81b83df07329f9fb28ce5b74f83267adde552cdd9e605325ab77`。

以下保留 v2.0.0 的发布候选前向结果作为历史基线。
