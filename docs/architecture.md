...
检索器不依赖大型向量数据库，默认采用可解释的词项排名：英文/数字/技术符号词项 + 中文二元字符片段 + 精确短语加权。对于已分析 bundle，它还会从相邻 `evidence.json` 读取双语术语，并将翻译后的主张、行动和实体描述映射回源 segment ID 进行证据加权。它适合作为 Agent 的第一阶段召回；Agent 再读取相邻块并进行语义判断。这样离线可测试，也避免强制外部服务。
